...
class TestOrderCollection(unittest.TestCase):
    ...
    def test_add_order(self) -> None:
        ...
        o = Order(self.symbol, 'BUY', Price(0.015, 5, 4) , '0.15', 'LIMIT')
        oc.add_order(o)
        assert len(oc._orders) == 1
        assert oc.orders[0] == o
        o = Order(self.symbol, 'SELL', Price(0.016, 5, 4) , '0.5', 'LIMIT')
        oc.add_order(o)
    ...
    def test_add_order_raises_assertion_error(self):
        oc = OrderCollection(self.symbol)
        o = Order(Symbol('BTC-USDT'), 'BUY', Price(0.015, 5, 4) , '0.15', 'LIMIT')
...
class TestCaseOrderCollection(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        o1 = Order(self.symbol, 'BUY', Price(40000,8,2), '1.0')
        o2 = Order(self.symbol, 'BUY', Price(20000,8,2), '1.0')
